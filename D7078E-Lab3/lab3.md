...
member’s contributions (More than one individual can work on similar tasks).
Lab Objectives:
...
Web App → DB (ONLY via HTTPS API, not direct SG rules)
App → S3 bucket (via API Gateway, not direct S3 endpoint)
...
distinct function:
1. Instance #1 – Web Application Server (WEB-SERVER)
2. Instance #2 – Database Server (DB-SERVER)
3. Instance #3 – Storage/API Server (STORAGE-SERVER)
Additionally:
• S3 bucket (for storage tasks)
• API Gateway (to expose controlled access to the S3 bucket)
• Security Groups (network-layer access control)
• IAM Roles & Policies (identity-layer access control)
3
Traffic Rules:
...
EC2 → EC2 traffic       None allowed                           Blocked by SG rules                           The three instances must remain isolated
SSH                     Only Admin → DB Server                 Admin PC → DB-SERVER port 22                  No other connections allowed
...
🧱 NETWORK RULES (Security Group Requirements)
1. SG-WEB (Security Group for Web Application Server)
Allowed:
HTTPS (443) from anywhere (0.0.0.0/0) — for external access to the web app
Optional: SSH from admin IP only
Denied:
❌ No traffic from DB Server
❌ No traffic from Storage Server
4
❌ No MySQL (3306) inbound
❌ No EC2-to-EC2 traffic allowed at all
❌ No outbound access to DB server
2. SG-DB (Security Group for Database Server)
Allowed:
SSH (22) only from administrator's IP
Example: 203.0.113.55/32
Denied:
❌ No inbound from Web Server
❌ No inbound from Storage Server
❌ No public HTTP/HTTPS
❌ No traffic from any EC2 instance
❌ MySQL must NOT be exposed publicly
❌ Web App cannot directly talk to the DB
...
API Gateway access happens outside the EC2 → it does NOT use SG-
STORAGE
...
The Storage Server will call API Gateway → API Gateway fetches objects from
S3.
1. Create REST API
2. Define a GET method → Integrate with S3
3. Deploy it
...
• Attempting to access an S3 bucket you do NOT have permissions for.
• Attempting to stop/terminate an EC2 instance using an IAM user without
that permission.
• Attempting to modify IAM roles or policies without necessary
permissions.
